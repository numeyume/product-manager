...
3. **🚨 重複チェック**: 在庫がある類似商品の購入時に自動警告
4. **在庫管理**: 「使用(-1)」ボタンで在庫を減らす
5. **場所管理**: 保管場所や利用場所を詳細に記録
...
- `DuplicateWarningModal`: 重複購入警告

### 重複防止システム
- `duplicateChecker`: AIベースの類似商品検出
- 商品名類似度判定 (60%重み)
- URL完全一致チェック (40%重み)
- 価格帯・サイト一致判定
- 在庫警告システム
...
- 🚨 **重複購入防止機能** - AIベースの類似商品検出
- 📊 **在庫警告システム** - 在庫ありの商品重複購入アラート
- 🎯 **スマート統合提案** - 既存商品への数量追加機能
- ✅ 利用場所の複数登録機能
- ✅ 商品編集での利用場所管理
- ✅ 使い方ガイドの追加
- ✅ ビジネスモード機能の実装
...
**検出アルゴリズム:**
```
商品名類似度: 60% (完全一致=100%, 部分一致=80%, 単語一致=可変)
URL完全一致: 40% (同じ商品URLの場合)
サイト一致: 10% (同じECサイト)
価格類似: 10% (±20%以内)
```
